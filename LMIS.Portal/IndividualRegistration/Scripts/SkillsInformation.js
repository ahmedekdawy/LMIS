function ViewModel() {
    ...
    self.LoadRecord = function () {
        ...
        lmis.ajax("../IndividualRegistration/SkillsInformation.aspx/GetNewSkills", { langCode: lmis.uiCulture }, 0, "show,close",
            function (data) {
                if (data.d) {
                    //Populate UI
                    self.Skills(data.d);
    ...
    self.Save = function (validateOnly, onSuccess, onError) {
        ...
        return lmis.ajax("../IndividualRegistration/SkillsInformation.aspx/PostNewSkills", dto, 0, "",
            function (data) {
                if (!validateOnly && data.d)
                    lmis.notification.success();
                window.location.assign("TrainingInformation.aspx?#anchor");
                onSuccess();
            },
            function (xhr) {
